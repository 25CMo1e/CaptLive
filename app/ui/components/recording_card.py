...
class RecordingCardManager:
    ...
    def _create_card_components(self, recording: Recording):
        ...
        # 临时设置display_title以便格式化方法使用
        recording.display_title = display_title
        display_title = self._format_display_title(recording)
    ...
    def _format_display_title(self, recording):
        """Format display title with timed recording prefix if enabled."""
        display_title = recording.display_title
        
        # 添加定时录制标识
        if recording.enabled_timed_recording:
            timed_text = recording.get_timed_recording_display_text()
            if timed_text:
                timed_prefix = f"[定时:{timed_text}] "
                # 移除可能存在的旧定时前缀
                if "[定时:" in display_title:
                    display_title = re.sub(r'\[定时:[^\]]+\]\s*', '', display_title)
                
                # 如果标题以其他前缀开始（如[停止监控中]），在它后面添加定时前缀
                if display_title.startswith("[") and "]" in display_title:
                    # 找到第一个]的位置
                    end_idx = display_title.find("]")
                    if end_idx > 0:
                        display_title = display_title[:end_idx+1] + " " + timed_prefix + display_title[end_idx+1:].strip()
                else:
                    display_title = timed_prefix + display_title
    ...
    async def update_duration(self, recording: Recording):
        ...
        while True:
            ...
            if recording.recording:
                ...
                # 检查定时录制是否到期（每秒检查一次，确保及时停止）
                if recording.enabled_timed_recording and recording.timed_recording_start_time:
                    total_minutes = recording.get_timed_recording_total_minutes()
                    if total_minutes > 0:
                        elapsed = datetime.now() - recording.timed_recording_start_time
                        ...
                        if elapsed_minutes >= total_minutes:
                            ...
                            self.app.record_manager.stop_recording(recording)
                            # 异步停止监控
                            self.app.page.run_task(self.app.record_manager.stop_monitor_recording, recording, True)
                            # 更新UI
                            self.app.page.run_task(self.app.record_card_manager.update_card, recording)
                            self.app.page.pubsub.send_others_on_topic("update", recording)
                            # 显示提示
                            timed_msg = self.app.record_manager._.get('timed_recording_completed', 'Timed recording completed')
                            self.app.page.run_task(
                                self.app.snack_bar.show_snack_bar,
                                f"{recording.title} {timed_msg}",
