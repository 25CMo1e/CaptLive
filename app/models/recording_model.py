...
class Recording:
    def __init__(
        ...
        enabled_barrage_recording=False,
        enabled_timed_recording=False,
        timed_recording_days=0,
        timed_recording_hours=0,
        timed_recording_minutes=0
    ):
        """
        ...
        :param enabled_barrage_recording: Whether to enable barrage recording.
        :param enabled_timed_recording: Whether to enable timed recording.
        :param timed_recording_days: Number of days for timed recording.
        :param timed_recording_hours: Number of hours for timed recording.
        :param timed_recording_minutes: Number of minutes for timed recording.
        """
        ...
        self.enabled_barrage_recording = enabled_barrage_recording
        self.enabled_timed_recording = enabled_timed_recording
        self.timed_recording_days = timed_recording_days or 0
        self.timed_recording_hours = timed_recording_hours or 0
        self.timed_recording_minutes = timed_recording_minutes or 0
        self.timed_recording_start_time = None  # 记录定时录制开始时间
    ...
    def to_dict(self):
        ...
        return {
            ...
            "enabled_barrage_recording": self.enabled_barrage_recording,
            "enabled_timed_recording": self.enabled_timed_recording,
            "timed_recording_days": self.timed_recording_days,
            "timed_recording_hours": self.timed_recording_hours,
            "timed_recording_minutes": self.timed_recording_minutes,
        }
    ...
    def from_dict(cls, data):
        ...
        recording = cls(
            ...
            data.get("enabled_barrage_recording", False),
            data.get("enabled_timed_recording", False),
            data.get("timed_recording_days", 0),
            data.get("timed_recording_hours", 0),
            data.get("timed_recording_minutes", 0),
        )
    ...
    def get_timed_recording_total_minutes(self):
        """Calculate total duration in minutes for timed recording."""
        if not self.enabled_timed_recording:
            return 0
        days = self.timed_recording_days or 0
        hours = self.timed_recording_hours or 0
        minutes = self.timed_recording_minutes or 0
        return days * 24 * 60 + hours * 60 + minutes
    
    def get_timed_recording_display_text(self):
        """Get formatted display text for timed recording duration."""
        if not self.enabled_timed_recording:
            return ""
        parts = []
        if self.timed_recording_days and self.timed_recording_days > 0:
            parts.append(f"{self.timed_recording_days}天")
        if self.timed_recording_hours and self.timed_recording_hours > 0:
            parts.append(f"{self.timed_recording_hours}小时")
        if self.timed_recording_minutes and self.timed_recording_minutes > 0:
            parts.append(f"{self.timed_recording_minutes}分钟")
