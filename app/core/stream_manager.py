...
class LiveStreamRecorder:
    ...
    async def start_recording(self, stream_info: StreamData):
        ...
        # 启动弹幕录制（在录制目录确定后）
        if self.recording.enabled_barrage_recording:
            from .barrage_manager import barrage_manager
            from .platform_handlers import get_platform_info
            _, platform_key = get_platform_info(self.live_url)
            if platform_key:
                barrage_manager.start_barrage_recording(
                    recording_id=self.recording.rec_id,
                    live_url=self.live_url,
                    platform=platform_key,
                    recording_dir=self.recording.recording_dir,
                    streamer_name=self.recording.streamer_name
                )
            ...
            if return_code in safe_return_code:
                ...
                if self.user_config.get("convert_to_mp4") and self.save_format == "ts":
                    if self.segment_record:
                        # 合并分段 .ts 为单个 mp4 文件后再删除原始 .ts（若配置要求）
                        directory = os.path.dirname(save_file_path)
                        prefix = os.path.basename(save_file_path).rsplit("_", maxsplit=1)[0]
                        try:
                            # 使用页面任务异步执行合并函数
                            self.app.page.run_task(
                                self.converts_ts_segments_to_mp4, directory, prefix, self.user_config["delete_original"]
                            )
                        except Exception as e:
                            logger.error(f"Failed to convert segmented video: {e}")
                            await self.converts_ts_segments_to_mp4(directory, prefix, self.user_config["delete_original"])
                    else:
                        try:
                            self.app.page.run_task(
    ...
    async def custom_script_execute(
        ...
        # TODO: 实现自定义脚本执行逻辑
        """将目录中以 prefix 开头的 .ts 段合并为单个 mp4，并在成功后删除原始 .ts 文件（如果配置要求）。"""
        try:
            directory = directory.replace("\\", "/")
            # 列出匹配的 .ts 文件并按文件名排序
            file_paths = [
                os.path.join(directory, f) for f in os.listdir(directory) if f.endswith('.ts') and prefix in f
            ]
            if not file_paths:
                logger.info(f"No TS segments found for prefix: {prefix} in {directory}")
                return

            file_paths.sort()

            # 生成 concat 列表文件
            list_file = os.path.join(directory, f"{prefix}_concat_list.txt")
            with open(list_file, 'w', encoding='utf-8') as lf:
                for p in file_paths:
                    # ffmpeg concat demuxer 要求路径被单引号包裹，如果包含特殊字符则使用绝对路径
                    lf.write(f"file '{p.replace("'", "'\\''")}'\n")

            output_mp4 = os.path.join(directory, f"{prefix}.mp4")
            ...
            try:
                ...
                logger.info(f"Merged TS segments to MP4: {output_mp4}")
                ...
                if is_original_delete:
                    ...
                    for p in file_paths:
                        try:
                            ...
                            logger.error(f"Failed to delete segment {p}: {e}")
                    try:
                        if os.path.exists(list_file):
                            os.remove(list_file)
                    except Exception as e:
                        logger.error(f"Failed to delete concat list {list_file}: {e}")
                    logger.info(f"Deleted original TS segments for prefix: {prefix}")
                else:
                    # 将原始文件移动到 original 子目录
                    converts_dir = os.path.join(directory, 'original')
                    os.makedirs(converts_dir, exist_ok=True)
                    for p in file_paths:
                        try:
                            shutil.move(p, converts_dir)
                        except Exception as e:
                            logger.error(f"Failed to move segment {p}: {e}")
                    try:
                        if os.path.exists(list_file):
                            shutil.move(list_file, converts_dir)
                    except Exception as e:
                        logger.error(f"Failed to move concat list {list_file}: {e}")

            except subprocess.CalledProcessError as e:
                logger.error(f"TS segments merge failed! Error message: {e.output.decode()}")

        except Exception as e:
            logger.error(f"An error occurred during TS segments merge: {e}")
        else:
            params = [
