...
class RecordingManager:
    ...
    async def check_all_live_status(self):
        ...
        # 检查定时录制是否到期
        await self.check_timed_recording()

    async def check_timed_recording(self):
        """Check if timed recording duration has been reached and stop recording/monitoring."""
        for recording in self.recordings:
            if recording.enabled_timed_recording and recording.recording and recording.timed_recording_start_time:
                ...
                if total_minutes > 0:
                    ...
                    if elapsed_minutes >= total_minutes:
                        logger.info(f"Timed recording duration reached for {recording.title}, stopping recording and monitoring")
                        # 停止录制
                        self.stop_recording(recording)
                        # 停止监控
                        await self.stop_monitor_recording(recording, auto_save=True)
                        # 更新UI
                        self.app.page.run_task(self.app.record_card_manager.update_card, recording)
                        self.app.page.pubsub.send_others_on_topic("update", recording)
                        # 显示提示
                        self.app.page.run_task(
                            ...
                            f"{recording.title} {self._['timed_recording_completed'] if 'timed_recording_completed' in self._ else 'Timed recording completed'}",
    ...
    def start_update(recording: Recording):
        ...
        if recording.is_live and not recording.recording:
            ...
            if recording.enabled_timed_recording:
                recording.timed_recording_start_time = now
    ...
    def stop_recording(recording: Recording):
        ...
        if recording.recording:
            ...
            # 停止弹幕录制
            if recording.enabled_barrage_recording:
                barrage_manager.stop_barrage_recording(recording.rec_id)
