...
class BarrageManager:
    ...
    def __init__(self):
        ...
        self.binary_files: Dict[str, str] = {}  # recording_id -> binary_file_path
        self.lock = threading.Lock()
    
    def start_barrage_recording(self, recording_id: str, live_url: str, platform: str, recording_dir: str, streamer_name: str = None) -> bool:
        """开始弹幕录制"""
        try:
            if platform != "douyin":
            ...
            # 创建弹幕文件
            barrage_file = self._create_barrage_file(recording_id, recording_dir, streamer_name)
            if not barrage_file:
                return False
            
            # 创建二进制弹幕文件
            binary_file = self._create_binary_file(recording_id, recording_dir, streamer_name)
            if not binary_file:
                return False
            ...
            fetcher = StreamCapDouyinFetcher(
                ...
                on_raw_message=lambda method, payload: self._handle_raw_message(recording_id, method, payload)  # 保存原始二进制数据
            )
            ...
            with self.lock:
                ...
                self.binary_files[recording_id] = binary_file
        ...
        try:
            with self.lock:
                if recording_id in self.active_fetchers:
                    ...
                    if recording_id in self.binary_files:
                        del self.binary_files[recording_id]
    ...
    def get_binary_file(self, recording_id: str) -> Optional[str]:
        """获取二进制弹幕文件路径"""
        with self.lock:
            return self.binary_files.get(recording_id)
    ...
    def _create_barrage_file(self, recording_id: str, recording_dir: str, streamer_name: str = None) -> Optional[str]:
        """创建弹幕文件"""
        try:
            ...
            safe_name = "".join(c for c in streamer_name if c.isalnum() or c in (' ', '-', '_')).rstrip() if streamer_name else recording_id
            filename = f"barrage_{safe_name}_{timestamp}.txt"
    ...
    def _create_binary_file(self, recording_id: str, recording_dir: str, streamer_name: str = None) -> Optional[str]:
        """创建二进制弹幕文件"""
        try:
            if not recording_dir:
                recording_dir = "downloads"
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            safe_name = "".join(c for c in streamer_name if c.isalnum() or c in (' ', '-', '_')).rstrip() if streamer_name else recording_id
            filename = f"barrage_{safe_name}_{timestamp}.bin"
            file_path = os.path.join(recording_dir, filename)
    ...
    def _handle_raw_message(self, recording_id: str, method: str, payload: bytes):
        """处理原始二进制消息"""
        try:
            if method == "WebSocketRawMessage":
                with self.lock:
                    if recording_id in self.binary_files:
                        binary_file = self.binary_files[recording_id]
                        with open(binary_file, "ab") as f:
                            f.write(payload)  # 直接写入原始二进制数据，无加工处理
        except Exception as e:
            logger.error(f"写入二进制弹幕文件失败: {e}")
